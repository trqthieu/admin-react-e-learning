...
const AddExercisePage: React.FC = () => {
  ...
  const handleEditOk = () => {
    editForm
      ...
      .then(async (values) => {
        ...
        setConfirmLoading(true);
        const editQuestionPayload: AddQuestionRequest = {
          title: values.title,
          description: values.description,
          content: values.content,
          answerType: values.answerType,
        ...
        const data = await updateQuestion(values.id, editQuestionPayload);
        if (data?.id) {
          notificationController.success({ message: t('common.success') });
        } else {
  ...
  const onDragEnd = async ({ active, over }: DragEndEvent) => {
    if (active.id !== over?.id) {
      ...
      if (over) {
        const data = await changeOrderQuestions({ activeId: +active.id, overId: +over.id });
        if (data?.affected) {
          fetch(initialPagination);
          ...
          <BaseCol xs={12} sm={12} xl={12}>
            <BaseCard id="validation form" title={'Manage questions'} padding="1.25rem">
              <BaseButton
                onClick={handleAdd}
                type="primary"
                style={{ marginBottom: 16, position: 'absolute', top: 20, right: 20 }}
              >
                Add a new question
              </BaseButton>
              <Modal
